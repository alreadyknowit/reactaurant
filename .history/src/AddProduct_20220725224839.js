...
class AddProduct extends Component {
  ...
  onClickHandler = async () => {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        categoryId: this.state.categoryId.categoryId,
      ...
      <div>
        <Form>
          ...
          <FormGroup>
            ...
            <Input name="categoryId" type="select" onChange={this.onChangeHandler}>
              {this.state.categories.map((category) => {
                return <option key={category.categoryId}>{category.categoryName}</option>;
              })}
            </Input>
          </FormGroup>
          <FormGroup>
        ...
        <h2>{this.state.categoryId.categoryId}</h2>
